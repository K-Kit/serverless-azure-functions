...
var Utils = /** @class */ (function () {
                ...
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        for (eventsIndex = 0; eventsIndex < events.length; eventsIndex++) {
                            ...
                            bindingUserSettings = {};
                            azureSettings = __assign(__assign({}, events[eventsIndex][constants_1.constants.xAzureSettings]), events[eventsIndex]);
    ...
    Utils.spawnLocal = function (options) {
        ...
        var localCommand = path_1.join(
        // @ts-ignore
        serverless.configurationInput.moduleRoot || serverless.config.servicePath, "node_modules", ".bin", command);
        return this.spawn(__assign(__assign({}, options), { command: localCommand, commandName: command }));
    };
    // public static spawn()
    Utils.spawn = function (options) {
        ...
        var env = __assign(__assign({}, process.env), serverless.service.provider["environment"]);
        if (!options.silent) {
